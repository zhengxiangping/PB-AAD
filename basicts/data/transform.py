...
class maskTransforms(object):
    ...
    def __call__(self, history_data):
        ...
        # 生成随机张量
        random_tensor = torch.rand_like(history_data)

        # 根据阈值生成随机位掩码
        mask = (random_tensor > threshold).float()

        # print(mask.shape)
        # print(mask)
        # print(history_data)
        history_data_mask = torch.mul(history_data, mask)
        # print(history_data)
        # dd

        # zero_matirx = torch.zeros_like(history_data)
        # history_data = self.mask_ratio
        # print()
        # history_data = 
        # # print('maskTransforms')
        return history_data, history_data_mask